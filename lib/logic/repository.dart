...
class Repository {
  ...
  Future<void> submitReview(int userId, int memberId, fsrs.Rating rating) async {
    ...
    final companion = FsrsReviewsCompanion.insert(
      ...
      elapsedDays: (nextCardMap['elapsedDays'] as int? ?? 0),
      scheduledDays: (nextCardMap['scheduledDays'] as int? ?? 0),
      reps: (nextCardMap['reps'] as int? ?? 0),
      lapses: (nextCardMap['lapses'] as int? ?? 0),
